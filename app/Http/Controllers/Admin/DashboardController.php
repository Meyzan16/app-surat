    ...
    public function index()
    {
        if(auth()->user()->roles == 'OPERATOR_PRODI')
        {
            ...
            $srt_umum = tb_loghistory_surat_umum::where('kode_prodi', auth()->user()->kode_prodi)
                                ->count();
        }elseif(auth()->user()->roles == 'VERIF_PERSETUJUAN' || auth()->user()->roles == 'KEPALA_OPERATOR')
        {
            $count_aktif_kuliah = tb_log_srt_ket_msh_kuliah::count();
            $count_ket_lulus = tb_log_ket_lulus::count();
            $srt_umum_mhs = tb_log_surat_mhs_umum::count();
            $srt_umum = tb_loghistory_surat_umum::count();
        }
